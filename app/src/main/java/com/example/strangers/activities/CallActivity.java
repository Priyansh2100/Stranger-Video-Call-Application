...
public class CallActivity extends AppCompatActivity {
    ...
    void setupWebView() {
        binding.webView.setWebChromeClient(new WebChromeClient() {
            @Override
            public void onPermissionRequest(PermissionRequest request) {  // jaise hamare call connect hogi to ek request jyegi hamare web view ke taraf se to application ko
    ...
    public void loadVideoCall() {
        String filePath = "file:android_asset/call.html";  // load the file path
    ...
    void initializePeer(){
        ...
        if(createdBy.equalsIgnoreCase(username)) {  // connect
            if(pageExit)
                return;
                    ...
                    FirebaseDatabase.getInstance().getReference()
                            ...
                            .addListenerForSingleValueEvent(new ValueEventListener() {
                                ...
                                public void onDataChange(@NonNull DataSnapshot snapshot) {
                                    User user = snapshot.getValue(User.class); // send call request
